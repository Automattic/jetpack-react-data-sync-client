...
class DataSyncError extends Error {
     ...
     */
    debugMessage() {
        if (this.errorData.error instanceof DOMException &&
            this.errorData.error.name === 'AbortError') {
            console.warn(`DataSync: ${this.errorData.method ?? '<unknown>'} ${this.errorData.location}  request aborted.`);
            return;
        }
        ...
        if (info.error instanceof zod_1.z.ZodError) {
            const msg = [];
            const messageStyle = [];
        ...
        if (this.errorData.namespace in window &&
            this.errorData.key in window[this.errorData.namespace]) {
            const value = window[this.errorData.namespace][this.errorData.key];
            console.log(`%cInitial Data%c:\nwindow.${key}.value =`, `${style.bold} ${style.spacing}`, '', value.value);
            if ('log' in value) {
                console.log(`%cPHP Log%c:`, `${style.bold} ${style.spacing}`, '', value.log.length > 0 ? value.log : 'No log messages.');
            }
            else {
                console.log(`%cPHP Log%c: PHP Log is disabled. To enable it, place the debug code in your wp-config.php:\n%cdefine('DATASYNC_DEBUG', true);`, `${style.bold} ${style.spacing}`, '', style.highlight);
            }
        }
        if (info.data !== undefined) {
            console.log(`%cRaw Data Received:%c\n`, `${style.bold} ${style.spacing}`, '', info.data);
        }
        console.groupEnd();
        console.groupEnd();
    }
...
exports.DataSyncError = DataSyncError;
//# sourceMappingURL=DataSyncError.js.map